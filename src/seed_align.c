...
/*struct contig_t
{
    uint64_t ref_start,ref_end;
    int read_start,read_end;
    int seed_start,seed_end;
    int score;
};*/
#define CONTIG_BUF_LENGTH (SEED_BUF_LENGTH/10)
               ...
               struct NW_list *NW1,struct NW_list *NW2,struct NW_list *NW3,struct Splice_DP_t *DP,struct splice_list *SP,int *SP_num)
{
    ...
    if(best[0].score>=(float)0.3*read->length*opt->match)
    {
        ...
        if(flag == 0)
        {
            ...
            generate_cigar(read,seed,&(seed_order[max_s]),seed_order_n,seed_a,result,result_num,read_order,out,un,NW1,NW2,NW3,DP,SP,SP_num);
        }
    }
...
void *seed_align_core(void* arg)
{
	...
	struct splice_list *SP = (struct splice_list *)malloc(opt->SP_block_MAX*sizeof(struct splice_list));
	int SP_num = 0;
	...
	while (1)
	{
        ...
        for (i = 0;i<read_num;i++)
        {
            ...
            if(seed_num!=0)
                find_cand(read+i,seed,&seed_num,seed_a,best,result,&result_num,i,job->out_file,job->un_file,NW1,NW2,NW3,DP,SP,&SP_num);
        }
    ...
    sp.end = 0;
    sp.num = 1;
	Update_SP_s(SP,&SP_num,opt->SP_block_MAX,&sp,1);
...
int seed_align(struct m_opt *opt)
{
    ...
    job->out_file = fopen(temp_name,"w");
