...
void check_exon(struct read_t *read0,char *name,char *read,int length,struct cigar_t *cigar,int *cigar_num,uint64_t *start_pos,int chr_order,struct Splice_DP_t *DP,char *cigarS)
{
    int break_flag = 1;
    int EXON_LENGTH = 50;
    ...
    for(i = 0;i<init_num;i++)
    {
        if((i+1<init_num)&&(seed[i+1].flag!=1))
        {
            seed[seed_num].ref_start = seed[front].ref_end+1;
            seed[seed_num].read_start = seed[i+1].read_start;
            seed[seed_num].ref_end = 0;
            seed[seed_num].read_end = seed[i+1].read_end;
            seed[seed_num].score = 0;

            t_num = 0;
            if(i+2<init_num)
            seed[seed_num].score = Tail_DP(opt->chr->list[chr_order].seq,seed[seed_num].ref_start,1,&(read[seed[seed_num].read_start]),(seed[seed_num].read_end-seed[seed_num].read_start+1),
                                               t_cigar,&t_num,DP,&(seed[seed_num].ref_end),0);
            else
                seed[seed_num].score = Tail_DP(opt->chr->list[chr_order].seq,seed[seed_num].ref_start,1,&(read[seed[seed_num].read_start]),(seed[seed_num].read_end-seed[seed_num].read_start+1),
                                               t_cigar,&t_num,DP,&(seed[seed_num].ref_end),1);

            if((seed[seed_num].score>=seed[i+1].score-opt->splice)||((i+1 == init_num-1)&&(seed[i+1].ref_end-seed[i+1].ref_start<opt->cut_tail)))
            {
                ...
                if(i+2>=init_num) back = front;
                else seed[i+2].front = front;
            }
            else front = i+1;
        }
        else front = i+1;
    ...
    while(i!=-1)
    {
        ...
        if((i!=-1)&&(seed[i].flag!=1))
        {
            ...
            t_num = 0;
            if(seed[i].front!=-1)
            seed[seed_num].score = Tail_DP(opt->chr->list[chr_order].seq,seed[seed_num].ref_end,0,&(read[seed[seed_num].read_start]),(seed[seed_num].read_end-seed[seed_num].read_start+1)
    ...
    for(i = 1;i<seed_order_n;i++)
    {
        ...
        //if((max_s==-1)||((max_s!=-1)&&(seed[seed_num].score>seed[max_s].score)))
        {
    ...
    for(i = 0;i<seed_order_n;i++)
    {
        ...
        for(j = seed[seed_order[i]].c_start;j<=seed[seed_order[i]].c_end;j++)
        {
                ...
                else
                {
                    ...
                    {
                        o = find_SP_Order(opt->SP,start+chr_start);
                        flag = 0;
                        pthread_mutex_lock(&UpdateLock);
                        while(opt->SP[o].start==start+chr_start)
                        {
                            if(opt->SP[o].end == start+chr_start+seed[seed_order[i]].cigar[j].l)
                            {
                                opt->SP[o].num++;
                                flag = 1;
                                break;
        ...
        //lengthS+=seed[seed_order[i]].read_end-seed[seed_order[i]].read_start+1-M;
        //check_cigar(name,cigarS,lengthS);
        //lengthS+=M;
    }
    /*for(i = 0;i<seed_order_n;i++)
    {
        ...
        for(j = seed[seed_order[i]].c_start;j<=seed[seed_order[i]].c_end;j++)
        {
                if((seed[seed_order[i]].cigar[j].c=='M')||(seed[seed_order[i]].cigar[j].c=='X')) M+=seed[seed_order[i]].cigar[j].l;
                else
                {
                    if(M!=0)
                    {
        ...
        if(i!=seed_order_n-1)
        {
            ...
            check_cigar2(seed[max_s].cigar,seed[max_s].cigar_num,seed[max_s].read_end-seed[max_s].read_start+1);
            start = seed[max_s].ref_start;
            for(j = 0;j<seed[max_s].cigar_num;j++)
            {
                if((seed[max_s].cigar[j].c=='M')||(seed[max_s].cigar[j].c=='X')) {M+=seed[max_s].cigar[j].l;start+=seed[max_s].cigar[j].l;}
                else
                {
                    if(M!=0)
                    {
                    ...
                    if((seed[max_s].cigar[j].l<=8)&&(seed[max_s].cigar[j].c=='N')) seed[max_s].cigar[j].c = 'D';
                    if((seed[max_s].cigar[j].l>8)&&(seed[max_s].cigar[j].c=='D')) seed[max_s].cigar[j].c = 'N';

                    if(seed[max_s].cigar[j].c == 'N')
                    {
                        o = find_SP_Order(opt->SP,start+chr_start);
                        flag = 0;
                        pthread_mutex_lock(&UpdateLock);
                        while(opt->SP[o].start==start+chr_start)
                        {
                            if(opt->SP[o].end == start+chr_start+seed[max_s].cigar[j].l)
                            {
                                opt->SP[o].num++;
                                flag = 1;
                                break;
                        ...
                        {
                            sp.start = start+chr_start;
                            sp.end = start+seed[max_s].cigar[j].l+chr_start;
                            sp.num = 1;
                            Update_SP(&sp);
                        }
                        pthread_mutex_unlock(&UpdateLock);

                        start+=seed[max_s].cigar[j].l;
                    }
                    else if(seed[max_s].cigar[j].c == 'D') start+=seed[max_s].cigar[j].l;

                    sprintf(temp,"%d%c",seed[max_s].cigar[j].l,seed[max_s].cigar[j].c);
                    strcat(cigarS,temp);
                }
            }
            //lengthS+=seed[max_s].read_end-seed[max_s].read_start+1-M;
            //check_cigar(name,cigarS,lengthS);
            //lengthS+=M;
        }
    }
               ...
               unsigned int **hash,int *hash_num,struct seed_t *seed,unsigned int *site,int align_flag,int chr_order,struct Splice_DP_t *DP)
{
    ...
    if(align_flag==1)
    {
        ...
        {
            Tail_DP(chr,start+1,1,seq,length,t_cigar,&t_num,DP,&start_pos,1);
            cigar2site(start+1,site,t_cigar,t_num);
        }
        else if(mode==1)//if((mode==1)&&(length<200))
        {
            Tail_DP(chr,end-1,0,seq,length,t_cigar,&t_num,DP,&start_pos,1);
...
void generate_cigar(struct read_t *read,struct seed_t *seed,int *seed_order,int seed_order_n,struct seed_t *seed_a,struct result_t *result,int *result_num,int read_order,FILE *out,FILE *un,struct NW_list *NW1,struct NW_list *NW2,struct NW_list *NW3,struct Splice_DP_t *DP)
{
    int i = 0,j = 0,x = 0,r = 0;
    ...
    if(seed[seed_order[0]].pos>=opt->l_pac)
    {
        ...
        {
            if(seed[seed_order[0]].pos <= (opt->l_pac<<1)-opt->chr->list[j].start_site-opt->chr->list[j].length){continue;}
            else break;
        }
        chr_order = j;
        strand = 1;
    ...
    else
    {
        ...
        check_exon(read-read_order,read->name,(strand==1)?read->rseq:read->seq,read->length,cigar,&cigar_num,&start_pos,chr_order,DP,cigarS);
