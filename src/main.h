...
extern void generate_cigar(struct read_t *read,struct seed_t *seed,int *seed_order,int seed_order_n,struct seed_t *seed_a,struct result_t *result,int *result_num,int read_order,FILE *out,FILE *un,struct NW_list *NW1,struct NW_list *NW2,struct NW_list *NW3,struct Splice_DP_t *DP);

extern int Tail_DP(char *chr,unsigned int start,int mode,char* string,int length,struct cigar_t *cigar,int *cigar_num,struct Splice_DP_t *DP,unsigned int *start_pos,int S);
